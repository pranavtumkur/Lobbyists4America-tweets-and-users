...
if os.path.exists('../SQL for data science/users_pickle.pkl'):
    df=pd.read_pickle('../SQL for data science/users_pickle.pkl')
    ...
    for c in range(rows_to_extract):
        ...
        start_from=y
    df.to_pickle('../SQL for data science/users_pickle.pkl')
else:
    for c in range(rows_to_extract):
        y=data.find('\n',start_from+1)
        if y>-1:
